...
def sort_recipes(breakfast_recipes ,lunch_recipes, dinner_recipes, user_caloric_intake):
    ...
    for day in weekly_recipe.keys():
        for recipe in breakfast_recipes:
            if recipe[1].nutrition['ENERC_KCAL']['value'] in range(breakfast_allotment - tolerance, breakfast_allotment + tolerance):
                weekly_recipe[day]["Breakfast"] = recipe
                breakfast_recipes.pop(0)
            break
        for recipe in lunch_recipes:
            if recipe[1].nutrition['ENERC_KCAL']['value'] in range(lunch_allotment - tolerance, lunch_allotment + tolerance):
                weekly_recipe[day]["Lunch"] = recipe
                lunch_recipes.pop(0)
            break
        for recipe in dinner_recipes:
            if recipe[1].nutrition['ENERC_KCAL']['value'] in range(dinner_allotment - tolerance, dinner_allotment + tolerance):
                weekly_recipe[day]["Dinner"] = recipe
                dinner_recipes.pop(0)
            break
